...
const navigationItems = ["Tjenester", "Prosess", "Om oss", "Kontakt"] as const;

export function Header() {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-sm">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center py-6">
        <div className="flex items-center gap-1">
          <div className="h-7 w-24 mb-1">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 375.21 131.49"
              preserveAspectRatio="xMidYMid meet"
              className="h-full w-full"
              aria-label="Guilty Labs Logo"
            >
              <title>Guilty Labs Logo</title>
          ...
          <span className="text-lg font-schoolbell text-primary">Labs</span>
        </div>
        <nav className="hidden md:flex space-x-8">
          {navigationItems.map((item) => (
            <a
              key={item}
              href="#"
              className="text-base font-light text-foreground hover:text-primary transition-colors"
            >
              {item}
            </a>
          ))}
        </nav>
        <Button className="font-light text-primary-foreground hover:bg-primary/90 tracking-wide">
          Book et møte
        </Button>
      </div>
    </header>
